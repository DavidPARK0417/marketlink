/**
 * @file components/admin/RetailerSuspensionForm.tsx
 * @description 소매사업자 정지/해제 폼 컴포넌트
 *
 * 관리자가 소매사업자를 정지하거나 해제할 수 있는 UI 컴포넌트입니다.
 ...
 *
 * @dependencies
 * - actions/admin/account-management.ts (suspendRetailer, unsuspendRetailer)
...
interface RetailerSuspensionFormProps {
  retailerId: string;
 ...
 * 소매사업자 정지/해제 폼 컴포넌트
 *
 * @param {RetailerSuspensionFormProps} props
 * @param {string} props.retailerId - 소매사업자 ID
 * @param {string} props.currentStatus - 현재 상태 ('active', 'suspended')
 */
export default function RetailerSuspensionForm({
  retailerId,
  currentStatus,
}: RetailerSuspensionFormProps) {
  ...
  const handleSuspend = async (data: SuspendFormData) => {
    ...
    try {
      ...
      await suspendRetailer(retailerId, data.suspensionReason);
      // 성공 시 페이지 새로고침
      router.refresh();
  ...
  const handleUnsuspend = async () => {
    ...
    try {
      console.log("🟢 [admin] 해제 버튼 클릭:", { retailerId });
    ...
    return (
      <div className="mt-8 bg-white dark:bg-gray-900 rounded-lg shadow border border-gray-200 dark:border-gray-800 hover:shadow-md hover:border-gray-300 dark:hover:border-gray-700 p-6 transition-colors duration-200">
        ...
        <Dialog open={isSuspendDialogOpen} onOpenChange={setIsSuspendDialogOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>소매사업자 계정 정지</DialogTitle>
              <DialogDescription>
