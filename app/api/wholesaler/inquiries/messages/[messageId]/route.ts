 ...
 * @description 문의 메시지 수정/삭제 API
 *
 * 도매사업자가 자신이 작성한 문의 메시지를 수정하거나 삭제하는 API 엔드포인트입니다.
 *
 * @dependencies
...
export async function DELETE(
  ...
  try {
    console.group(
      "🗑️ [api/wholesaler/inquiries/messages] 메시지 삭제 API 시작",
    );
    ...
    console.log("✅ [api/wholesaler/inquiries/messages] 메시지 삭제 성공");
    console.groupEnd();

    return NextResponse.json({ success: true });
  } catch (error) {
    console.error(
      "❌ [api/wholesaler/inquiries/messages] 메시지 삭제 오류:",
      error,
    );
